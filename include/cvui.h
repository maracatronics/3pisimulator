...
namespace cvui
{
	...
	*/
	...
	`OUT123` when the cursor is not over the iarea.
	...
	\return an integer value representing the current state of interaction with the mouse cursor. It can be `OUT123` (cursor is not over the area), `OVER` (cursor is over the area), `DOWN` (cursor is pressed over the area, but not released yet) and `CLICK` (cursor clicked, i.e. pressed and released, within the area).
...
namespace cvui
{
	...
	namespace internal
	{
		...
		bool button(cvui_block_t& theBlock, int theX, int theY, int theWidth, int theHeight, const cv::String& theLabel, bool theUpdateLayout) {
			...
			if (aMouseIsOver) {
				...
				render::button(theBlock, cvui::OUT123, aRect, theLabel);
				render::buttonLabel(theBlock, cvui::OUT123, aRect, theLabel, aTextSize);
			}
		...
		bool button(cvui_block_t& theBlock, int theX, int theY, cv::Mat& theIdle, cv::Mat& theOver, cv::Mat& theDown, bool theUpdateLayout) {
			...
			switch (aStatus) {
			case cvui::OUT123: render::image(theBlock, aRect, theIdle); break;
		...
		bool checkbox(cvui_block_t& theBlock, int theX, int theY, const cv::String& theLabel, bool *theState, unsigned int theColor) {
			...
			if (aMouseIsOver) {
				...
				render::checkbox(theBlock, cvui::OUT123, aRect);
			}
		...
		bool trackbar(cvui_block_t& theBlock, int theX, int theY, int theWidth, long double *theValue, const TrackbarParams & theParams) {
			...
			render::trackbar(theBlock, aMouseIsOver ? OVER : OUT123, aContentArea, *theValue, theParams);
	...
	namespace render
	{
		...
		void button(cvui_block_t& theBlock, int theState, cv::Rect& theShape, const cv::String& theLabel) {
			...
			// Inside
			theShape.x++; theShape.y++; theShape.width -= 2; theShape.height -= 2;
			cv::rectangle(theBlock.where, theShape, theState == OUT123 ? cv::Scalar(0x42, 0x42, 0x42) : (theState == OVER ? cv::Scalar(0x52, 0x52, 0x52) : cv::Scalar(0x32, 0x32, 0x32)), CVUI_FILLED);
		}
		...
		void checkbox(cvui_block_t& theBlock, int theState, cv::Rect& theShape) {
			// Outline
			cv::rectangle(theBlock.where, theShape, theState == OUT123 ? cv::Scalar(0x63, 0x63, 0x63) : cv::Scalar(0x80, 0x80, 0x80));
